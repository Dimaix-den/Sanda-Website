...
// Nav items: always link to /#hash so they work from any page
const navItems = [
  { label: 'Как работает', hash: 'features' },
  { label: 'Возможности', hash: 'features' },
  { label: 'Истории', hash: 'people' },
  { label: 'Сравнение', hash: 'compare' },
]
...
function NavLink({ hash, label, onClick }: { hash: string; label: string; onClick?: () => void }) {
  ...
  if (isHome) {
    // On home page — simple anchor, no navigation needed
    return (
      <a href={`#${hash}`} onClick={onClick} className="hover:text-text">
        {label}
      </a>
    )
  }
...
export function Header() {
  ...
  const swipe = useRef<{
    ...
    locked: 'x' | 'y' | null; width: number; pointerId: number | null
  ...
  function onPointerDown(e: React.PointerEvent) {
    ...
    if (!drawer) return
    swipe.current = { active: true, startX: e.clientX, startY: e.clientY, locked: null, width: drawer.offsetWidth, pointerId: e.pointerId }
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    if (!swipe.current.locked) swipe.current.locked = Math.abs(dx) > Math.abs(dy) ? 'x' : 'y'
  ...
  return (
    <>
        ...
        <div className="lg:sticky lg:top-0 lg:z-50">
          <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-3 md:px-5">
            ...
            <nav className="hidden items-center gap-8 text-sm text-text-muted lg:flex">
              {navItems.map((it) => (
                <NavLink key={it.label} hash={it.hash} label={it.label} />
              ))}
            </nav>
      ...
      <div
        ...
        className={`fixed inset-0 z-40 bg-black/70 transition-opacity duration-300 lg:hidden ${open ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`}
      />
      ...
      <aside
        ...
        className={`fixed inset-y-0 right-0 z-50 flex w-[88%] max-w-[360px] flex-col border-l border-line shadow-2xl transition-transform duration-300 ease-out lg:hidden ${open ? 'translate-x-0' : 'translate-x-full'}`}
      >
        ...
        >
          {/* BetaButton in drawer also opens the modal */}
          <BetaButton className="btn-primary w-full" onClick={() => setOpen(false)}>
            Скачать в TestFlight
          </BetaButton>
          <p className="mt-3 text-center text-xs text-text-dim">Бесплатно · iOS · Android (скоро)</p>
        </div>
      </aside>
    </>
  )
    ...
    <footer className="border-t border-line">
      <div className="mx-auto grid max-w-6xl grid-cols-2 gap-8 px-5 py-12 md:grid-cols-4 md:gap-10 md:py-14">
        <div className="col-span-2 md:col-span-1">
          <Logo />
          <p className="mt-4 max-w-xs text-sm text-text-muted">Одна цифра вместо таблиц. Спокойные деньги на каждый день.</p>
  ...
  return (
    <div>
      ...
      <ul className="mt-4 space-y-2.5 text-sm text-text-muted">
        {items.map((it) => (
          <li key={it.label}><a href={it.href} className="hover:text-text">{it.label}</a></li>
        ))}
      </ul>
    </div>
  )
